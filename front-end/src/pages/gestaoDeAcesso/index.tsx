...
import { GestoresApiResponse, ResponseAdminApi } from "../../types";
...
export default function GestaoDeAcesso() {
  ...
  const [filteredGestoresList, setFilteredGestoresList] = useState<
    ResponseAdminApi[]
  >([]);
  ...
  async function CadastarGestor() {
    try {
      const response = await cadastrarGestor(
        gestorNome,
        gestorMail,
        gestorPassword
      );
      if (response.status === 201) {
        setGestores((prevGestores) => [
          ...prevGestores,
          { key: response.data, value: gestorNome },
        ]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const gestoresUnicos: { key: string; value: string }[] = gestores.map(
          (gestor: GestoresApiResponse) => ({
            key: gestor.idGestor,
    ...
    <>
      <Stack spacing={3} margin="2% auto" marginLeft="25%">
        <Stack spacing={3}>
          ...
          <TabelaGestores
            gestores={
              selectedProjeto === "" ? gestoresList : filteredGestoresList
            }
          />
        </Stack>
        ...
        <Stack direction="row" spacing={10} pl={1}>
          <Stack spacing={3}>
            ...
            <Input
              ...
              handleInput={handleGestorNome}
              type="text"
            />
            <Input
              placeholder="email"
              handleInput={handleGestorMail}
              type="email"
            />
            <Input
              placeholder="senha"
              handleInput={handleGestorPassword}
              type="password"
            />

            <Button variant="outlined" onClick={() => CadastarGestor()}>
              Cadastrar
            </Button>
          </Stack>
          ...
          <Stack spacing={3}>
            ...
            <BasicSelect
              handlePesquisaByProjeto={selectedProjetoGestor}
              projeto={projeto}
              placeHolder="Selecione o Projeto"
            />
            <BasicSelect
              handlePesquisaByProjeto={selectedGestor}
              projeto={gestores}
